...
from composer.core import Evaluator, Algorithm, Event
...
def main(cfg):
    ...
    print(f"[Debugging] before FSDP is initialized, custom loading of the sparse checkpoint from {cfg.load_path}")
    model.load_state_dict(torch.load(cfg.load_path, map_location='cpu')['state']['model'], strict=True)
    for n, p in model.named_parameters():
        print(f"[Debugging] loaded {n}, shape = {p.shape}, sparsity = {torch.sum(p == 0)/p.numel()}")
    cfg.load_path = None  # disable loading from the load_path again
    ...
    algorithms.append(MaskPrunedWeights())  # here we add our custom algorithm to keep the mask
    ...
    trainer = Trainer(
        ...
        load_strict_model_weights=cfg.get('load_strict_model_weights', False),
